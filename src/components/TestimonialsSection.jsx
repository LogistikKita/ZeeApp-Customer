                ...
                {reviews.map((r, i) => (
                    <div key={i} className={`p-8 rounded-3xl relative ${darkMode ? 'bg-slate-800 border border-white/5' : 'bg-white shadow-xl shadow-gray-100'}`}>
                        ...
                        <div className="flex gap-1 mb-6">
                            {[...Array(r.rating || 5)].map((_, s) => <Star key={s} className="w-5 h-5 text-yellow-400 fill-yellow-400" />)}
                        </div>
                        <p className={`text-lg italic mb-8 leading-relaxed opacity-80 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>"{r.text}"</p>
                        <div className="flex items-center gap-4">
                            <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-xl text-white bg-gradient-to-br from-green-400 to-blue-500`}>
                                {r.name.charAt(0)}
                            </div>
                            <div>
                                <h4 className={`font-bold ${darkMode ? 'text-white' : 'text-slate-900'}`}>{r.name}</h4>
                                <p className="text-xs font-bold text-green-500 uppercase">{r.role}</p>
                            </div>
                        </div>
                    </div>
                ))}
